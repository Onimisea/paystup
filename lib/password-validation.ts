...
export function validatePassword(password: string): PasswordValidationResult {
  ...
  if (PASSWORD_REQUIREMENTS.requireUppercase && !/[A-Z]/.test(trimmedPassword)) {
    errors.push('Password must contain at least one uppercase letter (A-Z)');
  }

  // Check for lowercase letter
  if (PASSWORD_REQUIREMENTS.requireLowercase && !/[a-z]/.test(trimmedPassword)) {
  ...
  if (PASSWORD_REQUIREMENTS.requireSymbol) {
    const symbolRegex = new RegExp(`[${SUPPORTED_SYMBOLS.map(s => s === '\\' || s === ']' || s === '^' || s === '-' ? '\\' + s : s).join('')}]`);
    if (!symbolRegex.test(trimmedPassword)) {
      errors.push(`Password must contain at least one symbol: ${SUPPORTED_SYMBOLS.join(' ')}`);
    }
  }
  ...
  if (errors.length === 0) {
    const hasVariety = [
      ...
      new RegExp(`[${SUPPORTED_SYMBOLS.map(s => s === '\\' || s === ']' || s === '^' || s === '-' ? '\\' + s : s).join('')}]`).test(trimmedPassword)
...
export function processPassword(password: string): { trimmed: string; validation: PasswordValidationResult } {
